...
$$M_{ PA\ onset} = { St} \eta^3 M_{\star}$$

*Implementación:* Si $M_{ pl} < M_{ PA\ onset}$, el modelo no asume tasa nula, sino que cruza al modo de **Acreción de Planetesimales (Safronov)** (Ec. 7.14 de Ormel 2017), asumiendo régimen gas-free balístico bajo densidad propia rocosa. Esto produce que haya una transición real entre el crecimiento primigenio de embriones pequeños al runaway estrepitoso provisto por pebbles.
...
- **Headwind (Bondi-like drift limit)** ($M \lesssim M_{ hw/sh}$):
  $$\dot{M}_{ 2D, hw} = \sqrt{8 G M_{ pl} t_{ stop} v_{ hw}} \Sigma_{ peb}$$
- **Shear (Hill limit)** ($M \gtrsim M_{ hw/sh}$):
  $$\dot{M}_{ 2D, sh} = 2 R_{ Hill}^2 \Omega_{ K} { St}^{2/3} \Sigma_{ peb}$$
  
Con la masa de transición evaludaba matemáticamente con:

$$M_{ hw/sh} = \frac{v_{ hw}^3}{8 G \Omega_{ K} t_{ stop}}$$
...
$$\dot{M} = \dot{M}_{ 2D} \left( \frac{b_{ col}}{b_{ col} + h_{ peb} \sqrt{8/\pi}} \right)$$

Donde $h_{ peb} = \sqrt{\frac{\alpha_T}{\alpha_T + { St}}} h_{ gas}$. Esta transición converge al límite 3D natural en discos muy agitados verticalmente.
...
$$M_{ iso, peb} = 25 M_{\oplus} \left(\frac{H/r}{0.05}\right)^3 \left(\frac{M_{\star}}{M_{\odot}}\right)$$

### 5. Consumo y Drift
Los Pebbles viajan hacia la estrella limitados por:

$$v_{ r,solid} \approx \frac{- 2\eta v_{ K} { St}}{1 + { St}^2}$$
